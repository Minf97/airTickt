!function (e, t) {
  ...
  var r = i(0),
    ...
    return e.prototype.get = function (e, t) {
      ...
      var i = this.sandbox.get(e, t);
      if (i) return i;
      throw new ReferenceError(e + " is not defined");
    }, e.prototype.declare = function (e, t, i) {
      if (void 0 === i && (i = "var"), "var" === i) return this.varDeclare(e, t);
      if ("let" === i) return this.letDeclare(e, t);
      if ("const" === i) return this.constDeclare(e, t);
      throw new Error("evil-eval: Invalid Variable Declaration Kind");
    }, e.prototype.varDeclare = function (e, t) {
      for (var i = this; i.outer && "function" !== i.type;) i = i.outer;

      return this.declaration[e] = r.createSimpleValue(t, "var"), this.declaration[e];
    }, e.prototype.letDeclare = function (e, t) {
      if (this.declaration[e]) throw new SyntaxError("Identifier '" + e + "' has already been declared");
      return this.declaration[e] = r.createSimpleValue(t, "let"), this.declaration[e];
    }, e.prototype.constDeclare = function (e, t, i) {
      if (void 0 === i && (i = !1), !i && this.declaration[e]) throw new SyntaxError("Identifier '" + e + "' has already been declared");
  ...
  function a(e) {
    var t = e.node,
        i = function () {
      var i = e.createFunctionScope(!0);
      i.constDeclare("this", this), i.constDeclare("arguments", arguments);

      for (var r = 0, n = t.params.length; r < n; r++) {
        var a = t.params[r].name;
        i.varDeclare(a, arguments[r]);
      }

      var o = e.evaluate(t.body, {
        scope: i
      });
      if (s.default.isReturn(o)) return o.value;
    };
  ...
  t.Identifier = function (e) {
    ...
    for (var i = 0, r = e.node.body; i < r.length; i++) if ("FunctionDeclaration" === (c = r[i]).type) e.evaluate(c, {
      scope: t
    });else if ("VariableDeclaration" === c.type && "var" === c.kind) for (var n = 0, a = c.declarations; n < a.length; n++) {
      var o = a[n];
      t.varDeclare(o.id.name);
    }

    for (var h = 0, p = e.node.body; h < p.length; h++) {
      var c;

      if ("FunctionDeclaration" !== (c = p[h]).type) {
        var u = e.evaluate(c, {
          scope: t
        });
        if (s.default.isSignal(u)) return u;
      }
    }
    ...
    var t;
    return e.node.argument && (t = e.evaluate(e.node.argument)), s.default.Return(t);
    ...
    var t;
    return e.node.label && (t = e.node.label.name), s.default.Break(t);
  }, t.ContinueStatement = function (e) {
    var t;
    return e.node.label && (t = e.node.label.name), s.default.Continue(t);
    ...
    for (var t = e.evaluate(e.node.discriminant), i = !1, r = 0, n = e.node.cases; r < n.length; r++) {
      ...
      if (i || a.test && t !== e.evaluate(a.test) || (i = !0), i) {
        var o = e.evaluate(a);
        if (s.default.isBreak(o)) break;
        if (s.default.isContinue(o)) continue;
        if (s.default.isReturn(o)) return o;
      }
    }
    ...
    for (var t = 0, i = e.node.consequent; t < i.length; t++) {
      var r = i[t],
          n = e.evaluate(r);
      if (s.default.isSignal(n)) return n;
    }
    ...
    try {
      ...
      if (r) {
        var s = r.param,
            a = e.createBlockScope(!0);
        return a.letDeclare(s.name, t), e.evaluate(r, {
          scope: a
        });
      }
    ...
    for (; e.evaluate(e.node.test);) {
      ...
      if (s.default.isSignal(t)) {
        if (s.default.isBreak(t)) {
          if (!t.value || t.value === e.label) break;
        } else if (s.default.isContinue(t) && (!t.value || t.value === e.label)) continue;
    ...
    for (t.init && "VariableDeclaration" === t.init.type && (i = e.createBlockScope()), t.init && e.evaluate(t.init, {
      scope: i
      ...
      if (s.default.isSignal(r)) {
        if (s.default.isBreak(r)) {
          if (!r.value || r.value === e.label) break;
        } else if (s.default.isContinue(r) && (!r.value || r.value === e.label)) continue;
    ...
    if ("VariableDeclaration" === r.type) {
      ...
      t = o.declare(h.name, void 0, r.kind);
    } else {
      if ("Identifier" !== r.type) throw new Error('evil-eval: [ForInStatement] Unsupported left type "' + r.type + '"');
      t = o.get(r.name, !0);
    }

    for (var p in e.evaluate(n)) {
      t.v = p;
      var c = e.evaluate(a, {
        scope: o
      });

      if (s.default.isSignal(c)) {
        if (s.default.isBreak(c)) {
          if (!c.value || c.value === e.label) break;
        } else if (s.default.isContinue(c) && (!c.value || c.value === e.label)) continue;
    ...
    for (var t = 0, i = e.node.declarations; t < i.length; t++) {
      var r = i[t],
          n = r.id.name,
          s = r.init ? e.evaluate(r.init) : void 0;
      e.scope.declare(n, s);
    }
    ...
    for (var t = {}, i = 0, r = e.node.properties; i < r.length; i++) {
      var n = r[i],
          s = void 0;
      if ("Literal" === n.key.type) s = n.key.value;else {
        if ("Identifier" !== n.key.type) throw new Error('evil-eval: [ObjectExpression] Unsupported property key type "' + n.key.type + '"');
        s = n.key.name;
      }
  ...
  var o = {
    ...
    typeof: function (e) {
      if ("Identifier" !== e.node.argument.type) return typeof e.evaluate(e.node.argument);
    ...
    delete: function (e) {
      ...
      return "MemberExpression" === t.type ? delete e.evaluate(t.object)[c(t, e)] : "Identifier" !== t.type && ("Literal" === t.type || void 0);
    }
  };

  t.UnaryExpression = function (e) {
    return o[e.node.operator](e);
  };
  ...
  t.UpdateExpression = function (e) {
    var t = u(e.node.argument, e);
    return h[e.node.operator](t, e.node.prefix);
  }, t.BinaryExpressionOperatorEvaluateMap = {
    "==": function (e, t) {
      return e == t;
    },
    "!=": function (e, t) {
    ...
    return t.BinaryExpressionOperatorEvaluateMap[e.node.operator](i, r);
  }, t.AssignmentExpressionOperatorEvaluateMap = {
    "=": function (e, t) {
      return e.v = t;
    },
    "+=": function (e, t) {
    ...
    var i = e.node,
        r = u(i.left, e, "=" === i.operator);
    return t.AssignmentExpressionOperatorEvaluateMap[i.operator](r, e.evaluate(i.right));
  };
  var p = {
    "||": function (e, t) {
      return e || t;
  ...
  function u(e, t, i) {
    if (void 0 === i && (i = !1), "Identifier" === e.type) return t.scope.get(e.name, i);

    if ("MemberExpression" === e.type) {
      var r = t.evaluate(e.object),
          s = c(e, t);
      return n.createMemberValue(r, s);
    }
  ...
  t.LogicalExpression = function (e) {
    var t = e.evaluate(e.node.left),
        i = e.evaluate(e.node.right);
    return p[e.node.operator](t, i);
  }, t.MemberExpression = function (e) {
    return e.evaluate(e.node.object)[c(e.node, e)];
    ...
    var t,
        i = e.evaluate(e.node.callee),
        r = e.node.arguments.map(function (t) {
      return e.evaluate(t);
    });
    return "MemberExpression" == e.node.callee.type && (t = e.evaluate(e.node.callee.object)), i.apply(t, r);
    ...
    return t;
  }, t.getPropertyName = c, t.getIdentifierOrMemberExpressionValue = u;
}, function (e, t, i) {
  "use strict";
  ...
  var n = r(i(1)),
    ...
    return e.prototype.evaluate = function (t, i) {
      void 0 === i && (i = {});
      var r = new e(t, i.scope || this.scope, this.evaluateMap);
      r.label = i.label, r.extra = i.extra;
      var n = this.evaluateMap[t.type];
      if (!n) throw new Error('evil-eval: Node type "' + t.type + '" is not implemented');
      return n(r);
    }, e.prototype.createBlockScope = function (e) {
      void 0 === e && (e = !1);
      var t = new n.default("block", this.scope);
      return t.invasive = e, t;
    }, e.prototype.createFunctionScope = function (e) {
      void 0 === e && (e = !1);
      var t = new n.default("function", this.scope);
      return t.invasive = e, t;
    }, e;
  }();
  ...
  var s = n(i(4));
  t.BinaryExpressionOperatorEvaluateMap = r({}, s.BinaryExpressionOperatorEvaluateMap, {
    "**": function (e, t) {
      return Math.pow(e, t);
    }
  }), t.BinaryExpression = function (e) {
    var i = e.evaluate(e.node.left),
        r = e.evaluate(e.node.right);
    return t.BinaryExpressionOperatorEvaluateMap[e.node.operator](i, r);
  }, t.AssignmentExpressionOperatorEvaluateMap = r({}, s.AssignmentExpressionOperatorEvaluateMap, {
    "**=": function (e, t) {
      return (i = e).v = Math.pow(i.v, t);
      var i;
    }
  }), t.AssignmentExpression = function (e) {
    var i = e.node,
        r = s.getIdentifierOrMemberExpressionValue(i.left, e, "=" === i.operator);
    return t.AssignmentExpressionOperatorEvaluateMap[i.operator](r, e.evaluate(i.right));
  };
}, function (e, t, i) {
  "use strict";
  ...
  function o(e) {
    var t,
        i = e.node;
    i.superClass && (t = e.evaluate(i.superClass));
    var r = e.evaluate(i.body, {
      extra: {
        SuperClass: t
      }
    });
    return i.id && Object.defineProperty(r, "name", {
      value: i.id.name
    }), r;
  }

  function h(e) {
    if ("Identifier" === e.node.type) e.kind ? (e.scope.declare(e.node.name, e.v, e.kind), e.declarationNames && e.declarationNames.push(e.node.name)) : e.scope.get(e.node.name, !0).v = e.v;else if ("ObjectPattern" === e.node.type) !function (e) {
      for (var t = e.node, i = e.v, r = e.env, n = e.scope, s = e.kind, a = e.declarationNames, o = 0, p = t.properties; o < p.length; o++) {
        var c = p[o],
            u = void 0;
        u = c.computed ? r.evaluate(c.key) : "Identifier" === c.key.type ? c.key.name : c.key.value;
        var l = i[u];

        if ("Identifier" === c.value.type) {
          if (null === l || void 0 === l) throw new TypeError("Cannot destructure property `" + u + "` of 'undefined' or 'null'.");
          if (s) n.declare(c.value.name, l, s), a && a.push(c.value.name);else {
            var d = n.get(c.value.name, !0);
            d.v = l;
          }
        } else h({
          node: c.value,
      ...
      if ("ArrayPattern" !== e.node.type) throw new Error('evil-eval: Not support to declare pattern value of node type "' + e.node.type + '"');
      !function (e) {
        for (var t = e.node, i = e.v, r = e.env, n = e.scope, s = e.kind, o = e.declarationNames, p = 0, c = t.elements.length; p < c; p++) {
          var u = t.elements[p],
              l = i[p];

          if ("Identifier" === u.type) {
            if (void 0 === l) throw new TypeError("Cannot read property 'Symbol(Symbol.iterator)' of undefined");
            if (null === l) throw new TypeError("Cannot read property 'Symbol(Symbol.iterator)' of object");
            if (s) n.declare(u.name, l, s), o && o.push(u.name);else {
              var d = n.get(u.name, !0);
              d.v = l;
            }
          } else if ("RestElement" === u.type) {
            var f = u.argument.name;
            if (l = a.slice.call(i, p), s) n.declare(f, l, s), o && o.push(f);else {
              var d = n.get(f, !0);
              d.v = l;
            }
          } else h({
  ...
  t.ExpressionStatement = function (e) {
    ...
    for (var t = 0, i = e.node.declarations; t < i.length; t++) {
      var r = i[t],
          n = r.init ? e.evaluate(r.init) : void 0;
      h({
        node: r.id,
        v: n,
        env: e,
    ...
    for (var t = [], i = 0, r = e.node.elements; i < r.length; i++) {
      var n = r[i];
      "SpreadElement" !== n.type ? t.push(e.evaluate(n)) : t = t.concat(e.evaluate(n.argument));
    }
    ...
    for (var t = {}, i = 0, r = e.node.properties; i < r.length; i++) {
      var n = r[i],
          s = void 0;
      s = n.computed ? "Identifier" === n.key.type ? e.scope.get(n.key.name).v : e.evaluate(n.key) : "Identifier" === n.key.type ? n.key.name : n.key.value;
      var a = e.evaluate(n.value);
      if ("init" === n.kind) t[s] = a;else if ("get" === n.kind) Object.defineProperty(t, s, {
        get: a
      });else {
        if ("set" !== n.kind) throw new Error('evil-eval: [ObjectExpression] Unsupported property kind "' + n.kind + '"');
    ...
    return t;
  }, t.FunctionExpression = function (e) {
    var t = e.node,
        i = function () {
      var i = e.createFunctionScope(!0);
      i.constDeclare("this", this), i.constDeclare("arguments", arguments), e.extra && e.extra.SuperClass && (e.extra.isConstructor || e.extra.isStaticMethod ? i.constDeclare("@@evil-eval/super", e.extra.SuperClass) : e.extra.isMethod && i.constDeclare("@@evil-eval/super", e.extra.SuperClass.prototype));
    ...
    "MemberExpression" === e.node.callee.type ? t = "Super" !== e.node.callee.object.type ? e.evaluate(e.node.callee.object) : e.scope.get("this").v : e.extra && e.extra.isConstructor && (t = e.scope.get("this").v);
    return i.apply(t, r);
  }, t.ForOfStatement = function (e) {
    for (var t = e.node, i = t.left, r = t.right, n = t.body, a = e.scope, o = 0, p = e.evaluate(r); o < p.length; o++) {
      var c = p[o];
      "VariableDeclaration" === i.type ? ("let" !== i.kind && "const" !== i.kind || (a = e.createBlockScope(!0)), h({
        node: i.declarations[0].id,
        v: c,
        env: e,
        scope: a,
        kind: i.kind
      ...
      if (s.default.isSignal(u)) {
        if (s.default.isBreak(u)) {
          if (!u.value || u.value === e.label) break;
        } else if (s.default.isContinue(u) && (!u.value || u.value === e.label)) continue;

        return u;
      }
    }
  }, t.Super = function (e) {
    return e.scope.get("@@evil-eval/super").v;
  }, t.SpreadElement = function (e) {
    throw new Error("evil-eval: [SpreadElement] Should not happen");
  }, t.ArrowFunctionExpression = function (e) {
    var t = e.node,
        i = function () {
      for (var i = e.createFunctionScope(!0), r = 0, n = t.params.length; r < n; r++) {
        var a = t.params[r].name;
        i.varDeclare(a, arguments[r]);
      }
    ...
    t = n ? e.evaluate(n.value, {
      extra: r({}, e.extra, {
        isConstructor: !0
      })
    }) : function () {}, e.extra && e.extra.SuperClass && function (e, t) {
      function i() {
        this.constructor = e;
      }

      p(e, t), e.prototype = null === t ? Object.create(t) : (i.prototype = t.prototype, new i());
    }(t, e.extra.SuperClass);

    for (var s = {}, a = {}, o = 0, h = i; o < h.length; o++) {
      var c = h[o],
          u = c.key.name;
      c.computed && (u = e.scope.get(u).v);
      var l = void 0;
      "method" === c.kind ? c.static ? ((l = s[u]) && l.value || (l = {
        configurable: !0,
        enumerable: !0
      }), l.value = e.evaluate(c.value, {
        extra: r({}, e.extra, {
          isStaticMethod: !0
        })
      }), s[u] = l) : ((l = a[u]) && l.value || (l = {
        configurable: !0,
        enumerable: !0
      }), l.value = e.evaluate(c.value, {
        extra: r({}, e.extra, {
      ...
      }), a[u] = l) : "get" === c.kind ? c.static ? ((l = s[u]) && !l.value || (l = {
        configurable: !0,
        enumerable: !0
      }), l.get = e.evaluate(c.value), s[u] = l) : ((l = a[u]) && !l.value || (l = {
        configurable: !0,
        enumerable: !0
      }), l.get = e.evaluate(c.value), a[u] = l) : "set" === c.kind && (c.static ? ((l = s[u]) && !l.value || (l = {
        configurable: !0,
        enumerable: !0
      }), l.set = e.evaluate(c.value), s[u] = l) : ((l = a[u]) && !l.value || (l = {
        configurable: !0,
        enumerable: !0
      }), l.set = e.evaluate(c.value), a[u] = l));
    }

    return Object.defineProperties(t, s), Object.defineProperties(t.prototype, a), t;
  }, t.MethodDefinition = function (e) {
    throw new Error("evil-eval: [MethodDefinition] Should not happen");
  }, t.ClassDeclaration = function (e) {
    var t = o(e);
    return e.scope.constDeclare(e.node.id.name, t), t;
  }, t.ClassExpression = o, t.MetaProperty = function (e) {
    throw new Error('evil-eval: "' + e.node.type + '" not implemented');
    ...
    var t = e.node,
        i = e.scope,
        r = i.get("exports");
    if (!t.declaration) throw new Error('evil-eval: "' + e.node.type + '" not implemented');
    if ("VariableDeclaration" === t.declaration.type) for (var n = 0, s = t.declaration.declarations; n < s.length; n++) {
      var a = s[n],
          o = e.evaluate(a.init),
          p = [];
      h({
        ...
        env: e,
        scope: e.scope,
        kind: t.declaration.kind,
        declarationNames: p
      });

      for (var c = 0, u = p; c < u.length; c++) {
        var l = u[c];
        r.v[l] = i.get(l).v;
      }
    } else r.v[t.declaration.id.name] = e.evaluate(t.declaration);
  }, t.ExportSpecifier = function (e) {
    throw new Error('evil-eval: "' + e.node.type + '" not implemented');
  }, t.ExportDefaultDeclaration = function (e) {
    e.scope.get("exports").v.default = e.evaluate(e.node.declaration);
  ...
  function f(e, t) {
    return e < 65 ? 36 === e : e < 91 || (e < 97 ? 95 === e : e < 123 || (e <= 65535 ? e >= 170 && p.test(String.fromCharCode(e)) : !1 !== t && d(e, u)));
  }

  function m(e, t) {
    return e < 48 ? 36 === e : e < 58 || !(e < 65) && (e < 91 || (e < 97 ? 95 === e : e < 123 || (e <= 65535 ? e >= 170 && c.test(String.fromCharCode(e)) : !1 !== t && (d(e, u) || d(e, l)))));
  }
  ...
  var k = {
    num: new v("num", y),
    regexp: new v("regexp", y),
    string: new v("string", y),
    name: new v("name", y),
    eof: new v("eof"),
    bracketL: new v("[", {
      beforeExpr: !0,
  ...
  function R(e, t) {
    for (var i = 1, r = 0;;) {
      S.lastIndex = r;
      var n = S.exec(e);
      if (!(n && n.index < t)) return new T(i, t - r);
  ...
  function D(e) {
    ...
    for (var i in M) t[i] = e && N(e, i) ? e[i] : M[i];

    if (t.ecmaVersion >= 2015 && (t.ecmaVersion -= 2009), null == t.allowReserved && (t.allowReserved = t.ecmaVersion < 5), V(t.onToken)) {
      var r = t.onToken;
    ...
    return V(t.onComment) && (t.onComment = function (e, t) {
      return function (i, r, n, s, a, o) {
        ...
        e.locations && (h.loc = new O(this, a, o)), e.ranges && (h.range = [n, s]), t.push(h);
      };
    }(t, t.onComment)), t;
  }
  ...
  var U = function (e, t, i) {
    this.options = e = D(e), this.sourceFile = e.sourceFile, this.keywords = F(s[e.ecmaVersion >= 6 ? 6 : 5]);
    var n = "";

    if (!e.allowReserved) {
      for (var a = e.ecmaVersion; !(n = r[a]); a--);

      "module" == e.sourceType && (n += " await");
    }

    this.reservedWords = F(n);
    var o = (n ? n + " " : "") + r.strict;
    this.reservedWordsStrict = F(o), this.reservedWordsStrictBind = F(o + " " + r.strictBind), this.input = String(t), this.containsEsc = !1, this.loadPlugins(e.plugins), i ? (this.pos = i, this.lineStart = this.input.lastIndexOf("\n", i - 1) + 1, this.curLine = this.input.slice(0, this.lineStart).split(E).length) : (this.pos = this.lineStart = 0, this.curLine = 1), this.type = k.eof, this.value = null, this.start = this.end = this.pos, this.startLoc = this.endLoc = this.curPosition(), this.lastTokEndLoc = this.lastTokStartLoc = null, this.lastTokStart = this.lastTokEnd = this.pos, this.context = this.initialContext(), this.exprAllowed = !0, this.inModule = "module" === e.sourceType, this.strict = this.inModule || this.strictDirective(this.pos), this.potentialArrowAt = -1, this.inFunction = this.inGenerator = this.inAsync = !1, this.yieldPos = this.awaitPos = 0, this.labels = [], 0 === this.pos && e.allowHashBang && "#!" === this.input.slice(0, 2) && this.skipLineComment(2), this.scopeStack = [], this.enterFunctionScope(), this.regexpState = null;
  };
  ...
  W.isLet = function () {
    ...
    if (f(i, !0)) {
      for (var r = t + 1; m(this.input.charCodeAt(r), !0);) ++r;

      var n = this.input.slice(t, r);
    ...
    var e = A.exec(this.input),
        t = this.pos + e[0].length;
    return !(E.test(this.input.slice(this.pos, t)) || "function" !== this.input.slice(t, t + 8) || t + 8 != this.input.length && m(this.input.charAt(t + 8)));
  }, W.parseStatement = function (e, t, i) {
    var r,
        n = this.type,
        s = this.startNode();

    switch (this.isLet() && (n = k._var, r = "let"), n) {
      case k._break:
      case k._continue:
      ...
      case k._export:
      case k._import:
        return this.options.allowImportExportEverywhere || (t || this.raise(this.start, "'import' and 'export' may only appear at the top level"), this.inModule || this.raise(this.start, "'import' and 'export' may appear only with 'sourceType: module'")), n === k._import ? this.parseImport(s) : this.parseExport(s, i);

      default:
        if (this.isAsyncFunction()) return e || this.unexpected(), this.next(), this.parseFunctionStatement(s, !0);
        var a = this.value,
            o = this.parseExpression();
        return n === k.name && "Identifier" === o.type && this.eat(k.colon) ? this.parseLabeledStatement(s, a, o) : this.parseExpressionStatement(s, o);
    }
  }, W.parseBreakContinueStatement = function (e, t) {
    var i = "break" == t;
    this.next(), this.eat(k.semi) || this.insertSemicolon() ? e.label = null : this.type !== k.name ? this.unexpected() : (e.label = this.parseIdent(), this.semicolon());
    ...
    return this.next(), this.labels.push(H), e.body = this.parseStatement(!1), this.labels.pop(), this.expect(k._while), e.test = this.parseParenExpression(), this.options.ecmaVersion >= 6 ? this.eat(k.semi) : this.semicolon(), this.finishNode(e, "DoWhileStatement");
  }, W.parseForStatement = function (e) {
    this.next();
    var t = this.options.ecmaVersion >= 9 && this.inAsync && this.eatContextual("await") ? this.lastTokStart : -1;
    if (this.labels.push(H), this.enterLexicalScope(), this.expect(k.parenL), this.type === k.semi) return t > -1 && this.unexpected(t), this.parseFor(e, null);
    var i = this.isLet();

    if (this.type === k._var || this.type === k._const || i) {
      var r = this.startNode(),
          n = i ? "let" : this.value;
      return this.next(), this.parseVar(r, !0, n), this.finishNode(r, "VariableDeclaration"), !(this.type === k._in || this.options.ecmaVersion >= 6 && this.isContextual("of")) || 1 !== r.declarations.length || "var" !== n && r.declarations[0].init ? (t > -1 && this.unexpected(t), this.parseFor(e, r)) : (this.options.ecmaVersion >= 9 && (this.type === k._in ? t > -1 && this.unexpected(t) : e.await = t > -1), this.parseForIn(e, r));
    }

    var s = new q(),
        a = this.parseExpression(!0, s);
    return this.type === k._in || this.options.ecmaVersion >= 6 && this.isContextual("of") ? (this.options.ecmaVersion >= 9 && (this.type === k._in ? t > -1 && this.unexpected(t) : e.await = t > -1), this.toAssignable(a, !1, s), this.checkLVal(a), this.parseForIn(e, a)) : (this.checkExpressionErrors(s, !0), t > -1 && this.unexpected(t), this.parseFor(e, a));
  }, W.parseFunctionStatement = function (e, t) {
    ...
    var t;
    this.next(), e.discriminant = this.parseParenExpression(), e.cases = [], this.expect(k.braceL), this.labels.push(z), this.enterLexicalScope();

    for (var i = !1; this.type != k.braceR;) if (this.type === k._case || this.type === k._default) {
      var r = this.type === k._case;
      t && this.finishNode(t, "SwitchCase"), e.cases.push(t = this.startNode()), t.consequent = [], this.next(), r ? t.test = this.parseExpression() : (i && this.raiseRecoverable(this.lastTokStart, "Multiple default clauses"), i = !0, t.test = null), this.expect(k.colon);
  ...
  W.parseTryStatement = function (e) {
    if (this.next(), e.block = this.parseBlock(), e.handler = null, this.type === k._catch) {
      ...
      this.next(), this.expect(k.parenL), t.param = this.parseBindingAtom(), this.enterLexicalScope(), this.checkLVal(t.param, "let"), this.expect(k.parenR), t.body = this.parseBlock(!1), this.exitLexicalScope(), e.handler = this.finishNode(t, "CatchClause");
    }
    ...
    return this.next(), e.test = this.parseParenExpression(), this.labels.push(H), e.body = this.parseStatement(!1), this.labels.pop(), this.finishNode(e, "WhileStatement");
  ...
  }, W.parseLabeledStatement = function (e, t, i) {
    for (var r = 0, n = this.labels; r < n.length; r += 1) {
      n[r].name === t && this.raise(i.start, "Label '" + t + "' is already declared");
    }

    for (var s = this.type.isLoop ? "loop" : this.type === k._switch ? "switch" : null, a = this.labels.length - 1; a >= 0; a--) {
      var o = this.labels[a];
    ...
    }), e.body = this.parseStatement(!0), ("ClassDeclaration" == e.body.type || "VariableDeclaration" == e.body.type && "var" != e.body.kind || "FunctionDeclaration" == e.body.type && (this.strict || e.body.generator)) && this.raiseRecoverable(e.body.start, "Invalid labeled declaration"), this.labels.pop(), e.label = i, this.finishNode(e, "LabeledStatement");
  }, W.parseExpressionStatement = function (e, t) {
    return e.expression = t, this.semicolon(), this.finishNode(e, "ExpressionStatement");
  }, W.parseBlock = function (e) {
    void 0 === e && (e = !0);
    var t = this.startNode();

    for (t.body = [], this.expect(k.braceL), e && this.enterLexicalScope(); !this.eat(k.braceR);) {
  ...
  }, W.parseFunction = function (e, t, i, r) {
    this.initFunction(e), (this.options.ecmaVersion >= 9 || this.options.ecmaVersion >= 6 && !r) && (e.generator = this.eat(k.star)), this.options.ecmaVersion >= 8 && (e.async = !!r), t && (e.id = "nullableID" === t && this.type != k.name ? null : this.parseIdent(), e.id && this.checkLVal(e.id, "var"));
    var n = this.inGenerator,
        s = this.inAsync,
        a = this.yieldPos,
        o = this.awaitPos,
        h = this.inFunction;
    return this.inGenerator = e.generator, this.inAsync = e.async, this.yieldPos = 0, this.awaitPos = 0, this.inFunction = !0, this.enterFunctionScope(), t || (e.id = this.type == k.name ? this.parseIdent() : null), this.parseFunctionParams(e), this.parseFunctionBody(e, i), this.inGenerator = n, this.inAsync = s, this.yieldPos = a, this.awaitPos = o, this.inFunction = h, this.finishNode(e, t ? "FunctionDeclaration" : "FunctionExpression");
    ...
    for (i.body = [], this.expect(k.braceL); !this.eat(k.braceR);) {
      ...
      n && "MethodDefinition" === n.type && "constructor" === n.kind && (r && this.raise(n.start, "Duplicate constructor in the same class"), r = !0);
    }

    return e.body = this.finishNode(i, "ClassBody"), this.finishNode(e, t ? "ClassDeclaration" : "ClassExpression");
  }, W.parseClassMember = function (e) {
    var t = this;
    ...
    var i = this.startNode(),
      ...
      var n = t.start,
          s = t.startLoc;
      return !!t.eatContextual(e) && (!(t.type === k.parenL || r && t.canInsertSemicolon()) || (i.key && t.unexpected(), i.computed = !1, i.key = t.startNodeAt(n, s), i.key.name = e, t.finishNode(i.key, "Identifier"), !1));
    };

    i.kind = "method", i.static = r("static");
    var n = this.eat(k.star),
        s = !1;
    n || (this.options.ecmaVersion >= 8 && r("async", !0) ? (s = !0, n = this.options.ecmaVersion >= 9 && this.eat(k.star)) : r("get") ? i.kind = "get" : r("set") && (i.kind = "set")), i.key || this.parsePropertyName(i);
    var a = i.key;
    ...
    if (this.next(), this.eat(k.star)) return this.expectContextual("from"), this.type !== k.string && this.unexpected(), e.source = this.parseExprAtom(), this.semicolon(), this.finishNode(e, "ExportAllDeclaration");

    if (this.eat(k._default)) {
      var i;

      if (this.checkExport(t, "default", this.lastTokStart), this.type === k._function || (i = this.isAsyncFunction())) {
        var r = this.startNode();
        this.next(), i && this.next(), e.declaration = this.parseFunction(r, "nullableID", !1, i);
    ...
    var i = t.type;
    if ("Identifier" == i) this.checkExport(e, t.name, t.start);else if ("ObjectPattern" == i) for (var r = 0, n = t.properties; r < n.length; r += 1) {
      var s = n[r];
    ...
    return this.next(), this.type === k.string ? (e.specifiers = Q, e.source = this.parseExprAtom()) : (e.specifiers = this.parseImportSpecifiers(), this.expectContextual("from"), e.source = this.type === k.string ? this.parseExprAtom() : this.unexpected()), this.semicolon(), this.finishNode(e, "ImportDeclaration");
    ...
    if (this.type === k.name) {
      var i = this.startNode();
      if (i.local = this.parseIdent(), this.checkLVal(i.local, "let"), e.push(this.finishNode(i, "ImportDefaultSpecifier")), !this.eat(k.comma)) return e;
    }

    if (this.type === k.star) {
      var r = this.startNode();
      return this.next(), this.expectContextual("as"), r.local = this.parseIdent(), this.checkLVal(r.local, "let"), e.push(this.finishNode(r, "ImportNamespaceSpecifier")), e;
    }

    for (this.expect(k.braceL); !this.eat(k.braceR);) {
      if (t) t = !1;else if (this.expect(k.comma), this.afterTrailingComma(k.braceR)) break;
      var n = this.startNode();
      n.imported = this.parseIdent(!0), this.eatContextual("as") ? n.local = this.parseIdent() : (this.checkUnreserved(n.imported), n.local = n.imported), this.checkLVal(n.local, "let"), e.push(this.finishNode(n, "ImportSpecifier"));
    }
  ...
  K.toAssignable = function (e, t, i) {
    ...
    return this.next(), 6 === this.options.ecmaVersion && this.type !== k.name && this.unexpected(), e.argument = this.parseBindingAtom(), this.finishNode(e, "RestElement");
    ...
    for (var r = [], n = !0; !this.eat(e);) if (n ? n = !1 : this.expect(k.comma), t && this.type === k.comma) r.push(null);else {
      ...
      if (this.type === k.ellipsis) {
        ...
        this.parseBindingListItem(s), r.push(s), this.type === k.comma && this.raise(this.start, "Comma is not permitted after the rest element"), this.expect(e);
        break;
      }
  ...
  X.checkPropClash = function (e, t, i) {
    if (!(this.options.ecmaVersion >= 9 && "SpreadElement" === e.type || this.options.ecmaVersion >= 6 && (e.computed || e.method || e.shorthand))) {
      ...
      var s = e.kind;
      if (this.options.ecmaVersion >= 6) "__proto__" === r && "init" === s && (t.proto && (i && i.doubleProto < 0 ? i.doubleProto = n.start : this.raiseRecoverable(n.start, "Redefinition of __proto__ property")), t.proto = !0);else {
        var a = t[r = "$" + r];
        if (a) ("init" === s ? this.strict && a.init || a.get || a.set : a.init || a[s]) && this.raiseRecoverable(n.start, "Redefinition of property");else a = t[r] = {
          init: !1,
          get: !1,
          set: !1
        };
    ...
    if (this.type === k.comma) {
      ...
      for (s.expressions = [n]; this.eat(k.comma);) s.expressions.push(this.parseMaybeAssign(e, t));
    ...
    var r = !1,
        n = -1,
        s = -1;
    t ? (n = t.parenthesizedAssign, s = t.trailingComma, t.parenthesizedAssign = t.trailingComma = -1) : (t = new q(), r = !0);
    var a = this.start,
        o = this.startLoc;
    ...
    if (i && (h = i.call(this, h, a, o)), this.type.isAssign) {
      ...
      return p.operator = this.value, p.left = this.type === k.eq ? this.toAssignable(h, !1, t) : h, r || q.call(t), t.shorthandAssign = -1, this.checkLVal(h), this.next(), p.right = this.parseMaybeAssign(e), this.finishNode(p, "AssignmentExpression");
    }
    ...
    if (this.eat(k.question)) {
      var s = this.startNodeAt(i, r);
      return s.test = n, s.consequent = this.parseMaybeAssign(), this.expect(k.colon), s.alternate = this.parseMaybeAssign(e), this.finishNode(s, "ConditionalExpression");
    }
    ...
    return this.checkExpressionErrors(t) ? n : n.start == i && "ArrowFunctionExpression" === n.type ? n : this.parseExprOp(n, i, r, -1, e);
  }, X.parseExprOp = function (e, t, i, r, n) {
    ...
    if (null != s && (!n || this.type !== k._in) && s > r) {
      ...
      var h = this.start,
          p = this.startLoc,
          c = this.parseExprOp(this.parseMaybeUnary(null, !1), h, p, s, n),
          u = this.buildBinary(t, i, e, c, o, a);
      return this.parseExprOp(u, t, i, r, n);
    }

    return e;
  }, X.buildBinary = function (e, t, i, r, n, s) {
    ...
    var i,
        r = this.start,
        n = this.startLoc;
    if (this.inAsync && this.isContextual("await")) i = this.parseAwait(), t = !0;else if (this.type.prefix) {
    ...
    var t = this.start,
        ...
        n = "ArrowFunctionExpression" === r.type && ")" !== this.input.slice(this.lastTokStart, this.lastTokEnd);
    if (this.checkExpressionErrors(e) || n) return r;
    var s = this.parseSubscripts(r, t, i);
  ...
  }, X.parseSubscripts = function (e, t, i, r) {
    for (var n = this.options.ecmaVersion >= 8 && "Identifier" === e.type && "async" === e.name && this.lastTokEnd == e.end && !this.canInsertSemicolon() && "async" === this.input.slice(e.start, e.end), s = void 0;;) if ((s = this.eat(k.bracketL)) || this.eat(k.dot)) {
      ...
      a.object = e, a.property = s ? this.parseExpression() : this.parseIdent(!0), a.computed = !!s, s && this.expect(k.bracketR), e = this.finishNode(a, "MemberExpression");
      ...
      if (n && !this.canInsertSemicolon() && this.eat(k.arrow)) return this.checkPatternErrors(o, !1), this.checkYieldAwaitInDefaultParams(), this.yieldPos = h, this.awaitPos = p, this.parseArrowExpression(this.startNodeAt(t, i), c, !0);
      this.checkExpressionErrors(o, !0), this.yieldPos = h || this.yieldPos, this.awaitPos = p || this.awaitPos;
      var u = this.startNodeAt(t, i);
      u.callee = e, u.arguments = c, e = this.finishNode(u, "CallExpression");
    } else {
      if (this.type !== k.backQuote) return e;
      var l = this.startNodeAt(t, i);
      l.tag = e, l.quasi = this.parseTemplate({
        isTagged: !0
    ...
    switch (this.type) {
      case k._super:
        return this.inFunction || this.raise(this.start, "'super' outside of function or class"), t = this.startNode(), this.next(), this.type !== k.dot && this.type !== k.bracketL && this.type !== k.parenL && this.unexpected(), this.finishNode(t, "Super");
      ...
      case k.name:
        ...
        if (this.options.ecmaVersion >= 8 && !s && "async" === a.name && !this.canInsertSemicolon() && this.eat(k._function)) return this.parseFunction(this.startNodeAt(r, n), !1, !1, !0);

        if (i && !this.canInsertSemicolon()) {
          if (this.eat(k.arrow)) return this.parseArrowExpression(this.startNodeAt(r, n), [a], !1);
          if (this.options.ecmaVersion >= 8 && "async" === a.name && this.type === k.name && !s) return a = this.parseIdent(), !this.canInsertSemicolon() && this.eat(k.arrow) || this.unexpected(), this.parseArrowExpression(this.startNodeAt(r, n), [a], !0);
        }
      ...
      case k._true:
      case k._false:
        return (t = this.startNode()).value = this.type === k._null ? null : this.type === k._true, t.raw = this.type.keyword, this.next(), this.finishNode(t, "Literal");
      ...
      case k.bracketL:
        return t = this.startNode(), this.next(), t.elements = this.parseExprList(k.bracketR, !0, !0, e), this.finishNode(t, "ArrayExpression");
    ...
    if (this.options.ecmaVersion >= 6) {
      ...
      for (this.yieldPos = 0, this.awaitPos = 0; this.type !== k.parenR;) {
        if (p ? p = !1 : this.expect(k.comma), n && this.afterTrailingComma(k.parenR, !0)) {
          c = !0;
          break;
        }

        if (this.type === k.ellipsis) {
          s = this.start, h.push(this.parseParenItem(this.parseRestBinding())), this.type === k.comma && this.raise(this.start, "Comma is not permitted after the rest element");
          break;
        }
      ...
      var f = this.start,
          m = this.startLoc;
      if (this.expect(k.parenR), e && !this.canInsertSemicolon() && this.eat(k.arrow)) return this.checkPatternErrors(u, !1), this.checkYieldAwaitInDefaultParams(), this.yieldPos = l, this.awaitPos = d, this.parseParenArrowList(i, r, h);
      h.length && !c || this.unexpected(this.lastTokStart), s && this.unexpected(s), this.checkExpressionErrors(u, !0), this.yieldPos = l || this.yieldPos, this.awaitPos = d || this.awaitPos, h.length > 1 ? ((t = this.startNodeAt(a, o)).expressions = h, this.finishNodeAt(t, "SequenceExpression", f, m)) : t = h[0];
    ...
    return e;
  }, X.parseParenArrowList = function (e, t, i) {
  ...
  X.parseNew = function () {
    ...
    if (this.options.ecmaVersion >= 6 && this.eat(k.dot)) {
      ...
      return e.property = this.parseIdent(!0), ("target" !== e.property.name || i) && this.raiseRecoverable(e.property.start, "The only valid meta property for new is new.target"), this.inFunction || this.raiseRecoverable(e.start, "new.target can only be used in functions"), this.finishNode(e, "MetaProperty");
    }

    var r = this.start,
        n = this.startLoc;
    return e.callee = this.parseSubscripts(this.parseExprAtom(), r, n, !0), this.eat(k.parenL) ? e.arguments = this.parseExprList(k.parenR, this.options.ecmaVersion >= 8, !1) : e.arguments = Y, this.finishNode(e, "NewExpression");
  }, X.parseTemplateElement = function (e) {
    var t = e.isTagged,
        i = this.startNode();
    return this.type === k.invalidTemplate ? (t || this.raiseRecoverable(this.start, "Bad escape sequence in untagged template literal"), i.value = {
      raw: this.value,
      cooked: null
    }) : i.value = {
    ...
    for (i.quasis = [r]; !r.tail;) this.expect(k.dollarBraceL), i.expressions.push(this.parseExpression()), this.expect(k.braceR), i.quasis.push(r = this.parseTemplateElement({
      isTagged: t
    }));

    return this.next(), this.finishNode(i, "TemplateLiteral");
  }, X.isAsyncProp = function (e) {
    return !e.computed && "Identifier" === e.key.type && "async" === e.key.name && (this.type === k.name || this.type === k.num || this.type === k.string || this.type === k.bracketL || this.type.keyword || this.options.ecmaVersion >= 9 && this.type === k.star) && !E.test(this.input.slice(this.lastTokEnd, this.start));
    ...
    for (i.properties = [], this.next(); !this.eat(k.braceR);) {
      if (r) r = !1;else if (this.expect(k.comma), this.afterTrailingComma(k.braceR)) break;
    ...
    var i,
        ...
        s,
        a = this.startNode();
    if (this.options.ecmaVersion >= 9 && this.eat(k.ellipsis)) return e ? (a.argument = this.parseIdent(!1), this.type === k.comma && this.raise(this.start, "Comma is not permitted after the rest element"), this.finishNode(a, "RestElement")) : (this.type === k.parenL && t && (t.parenthesizedAssign < 0 && (t.parenthesizedAssign = this.start), t.parenthesizedBind < 0 && (t.parenthesizedBind = this.start)), a.argument = this.parseMaybeAssign(!1, t), this.type === k.comma && t && t.trailingComma < 0 && (t.trailingComma = this.start), this.finishNode(a, "SpreadElement"));
    this.options.ecmaVersion >= 6 && (a.method = !1, a.shorthand = !1, (e || t) && (n = this.start, s = this.startLoc), e || (i = this.eat(k.star)));
    var o = this.containsEsc;
    return this.parsePropertyName(a), !e && !o && this.options.ecmaVersion >= 8 && !i && this.isAsyncProp(a) ? (r = !0, i = this.options.ecmaVersion >= 9 && this.eat(k.star), this.parsePropertyName(a, t)) : r = !1, this.parsePropertyValue(a, e, i, r, n, s, t, o), this.finishNode(a, "Property");
  }, X.parsePropertyValue = function (e, t, i, r, n, s, a, o) {
    if ((i || r) && this.type === k.colon && this.unexpected(), this.eat(k.colon)) e.value = t ? this.parseMaybeDefault(this.start, this.startLoc) : this.parseMaybeAssign(!1, a), e.kind = "init";else if (this.options.ecmaVersion >= 6 && this.type === k.parenL) t && this.unexpected(), e.kind = "init", e.method = !0, e.value = this.parseMethod(i, r);else if (t || o || !(this.options.ecmaVersion >= 5) || e.computed || "Identifier" !== e.key.type || "get" !== e.key.name && "set" !== e.key.name || this.type == k.comma || this.type == k.braceR) this.options.ecmaVersion >= 6 && !e.computed && "Identifier" === e.key.type ? (this.checkUnreserved(e.key), e.kind = "init", t ? e.value = this.parseMaybeDefault(n, s, e.key) : this.type === k.eq && a ? (a.shorthandAssign < 0 && (a.shorthandAssign = this.start), e.value = this.parseMaybeDefault(n, s, e.key)) : e.value = e.key, e.shorthand = !0) : this.unexpected();else {
      (i || r) && this.unexpected(), e.kind = e.key.name, this.parsePropertyName(e), e.value = this.parseMethod(!1);
  ...
  }, X.parseArrowExpression = function (e, t, i) {
  ...
  }, X.parseFunctionBody = function (e, t) {
    ...
    if (i) e.body = this.parseMaybeAssign(), e.expression = !0, this.checkParams(e, !1);else {
      ...
      var a = this.labels;
      this.labels = [], n && (this.strict = !0), this.checkParams(e, !r && !n && !t && this.isSimpleParamList(e.params)), e.body = this.parseBlock(!1), e.expression = !1, this.adaptDirectivePrologue(e.body.body), this.labels = a;
    }
    ...
    for (var i = {}, r = 0, n = e.params; r < n.length; r += 1) {
      var s = n[r];
      this.checkLVal(s, "var", t ? null : i);
    }
  }, X.parseExprList = function (e, t, i, r) {
    for (var n = [], s = !0; !this.eat(e);) {
      if (s) s = !1;else if (this.expect(k.comma), t && this.afterTrailingComma(e)) break;
      var a = void 0;
      i && this.type === k.comma ? a = null : this.type === k.ellipsis ? (a = this.parseSpread(r), r && this.type === k.comma && r.trailingComma < 0 && (r.trailingComma = this.start)) : a = this.parseMaybeAssign(!1, r), n.push(a);
    }
    ...
    var e = this.startNode();
    return this.next(), this.type == k.semi || this.canInsertSemicolon() || this.type != k.star && !this.type.startsExpr ? (e.delegate = !1, e.argument = null) : (e.delegate = this.eat(k.star), e.argument = this.parseMaybeAssign()), this.finishNode(e, "YieldExpression");
  ...
  $.enterFunctionScope = function () {
    ...
    this.scopeStack.push(t), J(t.parentLexical, e.lexical, e.parentLexical);
  }, $.exitLexicalScope = function () {
    var e = this.scopeStack.pop(),
        t = this.scopeStack[this.scopeStack.length - 1];
    J(t.childVar, e.var, e.childVar);
  }, $.canDeclareVarName = function (e) {
    var t = this.scopeStack[this.scopeStack.length - 1];
    return !N(t.lexical, e) && !N(t.parentLexical, e);
  }, $.canDeclareLexicalName = function (e) {
    var t = this.scopeStack[this.scopeStack.length - 1];
    return !N(t.lexical, e) && !N(t.var, e) && !N(t.childVar, e);
  }, $.declareVarName = function (e) {
    this.scopeStack[this.scopeStack.length - 1].var[e] = !0;
  }, $.declareLexicalName = function (e) {
    this.scopeStack[this.scopeStack.length - 1].lexical[e] = !0;
  };

  var ee = function (e, t, i) {
    this.type = "", this.start = t, this.end = 0, e.options.locations && (this.loc = new O(e, i)), e.options.directSourceFile && (this.sourceFile = e.options.directSourceFile), e.options.ranges && (this.range = [t, 0]);
  },
      te = U.prototype;
  ...
  }, te.finishNodeAt = function (e, t, i, r) {
    return ie.call(this, e, t, i, r);
  };
  ...
  se.initialContext = function () {
    ...
    return t === ne.f_expr || t === ne.f_stat || (e !== k.colon || t !== ne.b_stat && t !== ne.b_expr ? e === k._return || e == k.name && this.exprAllowed ? E.test(this.input.slice(this.lastTokEnd, this.start)) : e === k._else || e === k.semi || e === k.eof || e === k.parenR || e == k.arrow || (e == k.braceL ? t === ne.b_stat : e != k._var && e != k.name && !this.exprAllowed) : !t.isExpr);
    ...
    var t,
        i = this.type;
    i.keyword && e == k.dot ? this.exprAllowed = !1 : (t = i.updateContext) ? t.call(this, e) : this.exprAllowed = i.beforeExpr;
  }, k.parenR.updateContext = k.braceR.updateContext = function () {
    if (1 != this.context.length) {
      var e = this.context.pop();
      e === ne.b_stat && "function" === this.curContext().token && (e = this.context.pop()), this.exprAllowed = !e.isExpr;
    ...
    e.beforeExpr && e !== k.semi && e !== k._else && (e !== k.colon && e !== k.braceL || this.curContext() !== ne.b_stat) ? this.context.push(ne.f_expr) : this.context.push(ne.f_stat), this.exprAllowed = !1;
  ...
  Array.prototype.push.apply(ae.$LONE, ae.General_Category), ae.gc = ae.General_Category, ae.sc = ae.Script_Extensions = ae.scx = ae.Script;
  ...
  he.prototype.reset = function (e, t, i) {
    var r = -1 !== i.indexOf("u");
    this.start = 0 | e, this.source = t + "", this.flags = i, this.switchU = r && this.parser.options.ecmaVersion >= 6, this.switchN = r && this.parser.options.ecmaVersion >= 9;
  }, he.prototype.raise = function (e) {
    this.parser.raiseRecoverable(this.start, "Invalid regular expression: /" + this.source + "/: " + e);
  }, he.prototype.at = function (e) {
    var t = this.source,
        i = t.length;
    if (e >= i) return -1;
    var r = t.charCodeAt(e);
    return !this.switchU || r <= 55295 || r >= 57344 || e + 1 >= i ? r : (r << 10) + t.charCodeAt(e + 1) - 56613888;
  }, he.prototype.nextIndex = function (e) {
    var t = this.source,
        i = t.length;
    if (e >= i) return i;
    var r = t.charCodeAt(e);
    return !this.switchU || r <= 55295 || r >= 57344 || e + 1 >= i ? e + 1 : e + 2;
  }, he.prototype.current = function () {
    return this.at(this.pos);
  }, he.prototype.lookahead = function () {
    return this.at(this.nextIndex(this.pos));
  }, he.prototype.advance = function () {
    this.pos = this.nextIndex(this.pos);
  }, he.prototype.eat = function (e) {
    return this.current() === e && (this.advance(), !0);
  }, oe.validateRegExpFlags = function (e) {
    for (var t = e.validFlags, i = e.flags, r = 0; r < i.length; r++) {
      var n = i.charAt(r);
      -1 == t.indexOf(n) && this.raise(e.start, "Invalid regular expression flag"), i.indexOf(n, r + 1) > -1 && this.raise(e.start, "Duplicate regular expression flag");
    }
    ...
    var t = e.current();
    return !!ce(t) && (e.lastIntValue = t, e.advance(), !0);
  }, oe.regexp_eatPatternCharacters = function (e) {
    for (var t = e.pos, i = 0; -1 !== (i = e.current()) && !ce(i);) e.advance();
    ...
    var t = e.pos,
        i = e.current();
    return e.advance(), 92 === i && this.regexp_eatRegExpUnicodeEscapeSequence(e) && (i = e.lastIntValue), function (e) {
    ...
    return 48 === e.current() && !fe(e.lookahead()) && (e.lastIntValue = 0, e.advance(), !0);
    ...
    var t = e.current();
    return !!ue(t) && (e.lastIntValue = t % 32, e.advance(), !0);
  }, oe.regexp_eatRegExpUnicodeEscapeSequence = function (e) {
    var t,
        i = e.pos;
    ...
    if (e.eat(117)) {
      ...
      e.switchU && e.raise("Invalid unicode escape"), e.pos = i;
    }
    ...
    if (this.regexp_eatUnicodePropertyName(e) && e.eat(61)) {
      ...
      if (this.regexp_eatUnicodePropertyValue(e)) {
        ...
        return this.regexp_validateUnicodePropertyNameAndValue(e, i, r), !0;
      }
    }
    ...
    ae.hasOwnProperty(t) && -1 !== ae[t].indexOf(i) || e.raise("Invalid property name");
  }, oe.regexp_validateUnicodePropertyNameOrValue = function (e, t) {
    -1 === ae.$LONE.indexOf(t) && e.raise("Invalid property name");
  }, oe.regexp_eatUnicodePropertyName = function (e) {
    var t = 0;

    for (e.lastStringValue = ""; le(t = e.current());) e.lastStringValue += pe(t), e.advance();

    return "" !== e.lastStringValue;
  }, oe.regexp_eatUnicodePropertyValue = function (e) {
    var t = 0;

    for (e.lastStringValue = ""; de(t = e.current());) e.lastStringValue += pe(t), e.advance();
    ...
    return !1;
  }, oe.regexp_classRanges = function (e) {
    ...
    if (e.eat(92)) {
      ...
      if (e.switchU) {
        var i = e.current();
        (99 === i || xe(i)) && e.raise("Invalid class escape"), e.raise("Invalid escape");
      }
    ...
    var t = e.current();
    return !(!fe(t) && 95 !== t) && (e.lastIntValue = t % 32, e.advance(), !0);
    ...
    for (e.lastIntValue = 0; fe(i = e.current());) e.lastIntValue = 10 * e.lastIntValue + (i - 48), e.advance();
    ...
    for (e.lastIntValue = 0; me(i = e.current());) e.lastIntValue = 16 * e.lastIntValue + ve(i), e.advance();
    ...
    var t = e.current();
    return xe(t) ? (e.lastIntValue = t - 48, e.advance(), !0) : (e.lastIntValue = 0, !1);
    ...
    for (var r = 0; r < t; ++r) {
      var n = e.current();
      if (!me(n)) return e.pos = i, !1;
      e.lastIntValue = 16 * e.lastIntValue + ve(n), e.advance();
    }
  ...
  var ge = function (e) {
    this.type = e.type, this.value = e.value, this.start = e.start, this.end = e.end, e.options.locations && (this.loc = new O(e, e.startLoc, e.endLoc)), e.options.ranges && (this.range = [e.start, e.end]);
  },
      ye = U.prototype;
  ...
  ye.next = function () {
  ...
  }, "undefined" != typeof Symbol && (ye[Symbol.iterator] = function () {
    var e = this;
    return {
      next: function () {
        var t = e.getToken();
    ...
    return e && e.preserveSpace || this.skipSpace(), this.start = this.pos, this.options.locations && (this.startLoc = this.curPosition()), this.pos >= this.input.length ? this.finishToken(k.eof) : e.override ? e.override(this) : void this.readToken(this.fullCharCodeAtPos());
    ...
    if (-1 === r && this.raise(this.pos - 2, "Unterminated comment"), this.pos = r + 2, this.options.locations) for (S.lastIndex = i; (e = S.exec(this.input)) && e.index < this.pos;) ++this.curLine, this.lineStart = e.index + e[0].length;
    ...
    for (var t = this.pos, i = this.options.onComment && this.curPosition(), r = this.input.charCodeAt(this.pos += e); this.pos < this.input.length && !w(r);) r = this.input.charCodeAt(++this.pos);
    ...
    return this.options.ecmaVersion >= 6 && 46 === e && 46 === t ? (this.pos += 3, this.finishToken(k.ellipsis)) : (++this.pos, this.finishToken(k.dot));
  }, ye.readToken_slash = function () {
    var e = this.input.charCodeAt(this.pos + 1);
    return this.exprAllowed ? (++this.pos, this.readRegexp()) : 61 === e ? this.finishOp(k.assign, 2) : this.finishOp(k.slash, 1);
  }, ye.readToken_mult_modulo_exp = function (e) {
    var t = this.input.charCodeAt(this.pos + 1),
        i = 1,
        r = 42 === e ? k.star : k.modulo;
    return this.options.ecmaVersion >= 7 && 42 == e && 42 === t && (++i, r = k.starstar, t = this.input.charCodeAt(this.pos + 2)), 61 === t ? this.finishOp(k.assign, i + 1) : this.finishOp(r, i);
  }, ye.readToken_pipe_amp = function (e) {
    var t = this.input.charCodeAt(this.pos + 1);
    return t === e ? this.finishOp(124 === e ? k.logicalOR : k.logicalAND, 2) : 61 === t ? this.finishOp(k.assign, 2) : this.finishOp(124 === e ? k.bitwiseOR : k.bitwiseAND, 1);
    ...
    var t = this.input.charCodeAt(this.pos + 1);
    return t === e ? 45 != t || this.inModule || 62 != this.input.charCodeAt(this.pos + 2) || 0 !== this.lastTokEnd && !E.test(this.input.slice(this.lastTokEnd, this.pos)) ? this.finishOp(k.incDec, 2) : (this.skipLineComment(3), this.skipSpace(), this.nextToken()) : 61 === t ? this.finishOp(k.assign, 2) : this.finishOp(k.plusMin, 1);
  }, ye.readToken_lt_gt = function (e) {
    var t = this.input.charCodeAt(this.pos + 1),
        i = 1;
    return t === e ? (i = 62 === e && 62 === this.input.charCodeAt(this.pos + 2) ? 3 : 2, 61 === this.input.charCodeAt(this.pos + i) ? this.finishOp(k.assign, i + 1) : this.finishOp(k.bitShift, i)) : 33 != t || 60 != e || this.inModule || 45 != this.input.charCodeAt(this.pos + 2) || 45 != this.input.charCodeAt(this.pos + 3) ? (61 === t && (i = 2), this.finishOp(k.relational, i)) : (this.skipLineComment(4), this.skipSpace(), this.nextToken());
  }, ye.readToken_eq_excl = function (e) {
    var t = this.input.charCodeAt(this.pos + 1);
    return 61 === t ? this.finishOp(k.equality, 61 === this.input.charCodeAt(this.pos + 2) ? 3 : 2) : 61 === e && 62 === t && this.options.ecmaVersion >= 6 ? (this.pos += 2, this.finishToken(k.arrow)) : this.finishOp(61 === e ? k.eq : k.prefix, 1);
    ...
    for (var e, t, i = this.pos;;) {
      ...
      if (E.test(r) && this.raise(i, "Unterminated regular expression"), e) e = !1;else {
    ...
    return null == t && this.raise(this.start + 2, "Expected number in radix " + e), f(this.fullCharCodeAtPos()) && this.raise(this.pos, "Identifier directly after number"), this.finishToken(k.num, t);
  }, ye.readNumber = function (e) {
    var t = this.pos;
    e || null !== this.readInt(10) || this.raise(t, "Invalid number");
    var i = this.pos - t >= 2 && 48 === this.input.charCodeAt(t);
    i && this.strict && this.raise(t, "Invalid number"), i && /[89]/.test(this.input.slice(t, this.pos)) && (i = !1);
    var r = this.input.charCodeAt(this.pos);
    46 !== r || i || (++this.pos, this.readInt(10), r = this.input.charCodeAt(this.pos)), 69 !== r && 101 !== r || i || (43 !== (r = this.input.charCodeAt(++this.pos)) && 45 !== r || ++this.pos, null === this.readInt(10) && this.raise(t, "Invalid number")), f(this.fullCharCodeAtPos()) && this.raise(this.pos, "Identifier directly after number");
    ...
    for (var t = "", i = ++this.pos;;) {
      ...
      var r = this.input.charCodeAt(this.pos);
      if (r === e) break;
      92 === r ? (t += this.input.slice(i, this.pos), t += this.readEscapedChar(!1), i = this.pos) : (w(r) && this.raise(this.start, "Unterminated string constant"), ++this.pos);
    }
  ...
  ye.tryReadTemplateToken = function () {
    ...
    for (var e = "", t = this.pos;;) {
      ...
      if (96 === i || 36 === i && 123 === this.input.charCodeAt(this.pos + 1)) return this.pos !== this.start || this.type !== k.template && this.type !== k.invalidTemplate ? (e += this.input.slice(t, this.pos), this.finishToken(k.template, e)) : 36 === i ? (this.pos += 2, this.finishToken(k.dollarBraceL)) : (++this.pos, this.finishToken(k.backQuote));
      if (92 === i) e += this.input.slice(t, this.pos), e += this.readEscapedChar(!0), t = this.pos;else if (w(i)) {
    ...
    for (var e = "", t = !0, i = this.pos, r = this.options.ecmaVersion >= 6; this.pos < this.input.length;) {
      ...
      if (m(n, r)) this.pos += n <= 65535 ? 1 : 2;else {
        if (92 !== n) break;
        this.containsEsc = !0, e += this.input.slice(i, this.pos);
        var s = this.pos;
        117 != this.input.charCodeAt(++this.pos) && this.invalidStringToken(this.pos, "Expecting Unicode escape sequence \\uXXXX"), ++this.pos;
        var a = this.readCodePoint();
        (t ? f : m)(a, r) || this.invalidStringToken(s, "Invalid Unicode escape"), e += be(a), i = this.pos;
      }
      t = !1;
    }
  ...
  var a = n(i(13)),
    ...
    function e(e, t) {
      void 0 === t && (t = {}), this.options = r({
        ecmaVersion: 5,
        sourceType: "module"
      }, t), this.options.ecmaVersion = d[this.options.ecmaVersion], this.code = e, this.ast = a.parse(this.code, this.options), this.evaluateMap = o.default[this.options.ecmaVersion];
    }

    return e.prototype.runInContext = function (e) {
      void 0 === e && (e = {});
      var t = this.createGlobalScope(e),
          i = new h.default(null, t, this.evaluateMap);
      ...
      if ("module" === this.options.sourceType) {
        ...
        return t.declaration.exports = l.createSimpleValue(r), t.declaration.module = l.createSimpleValue(n), i.evaluate(this.ast), n.exports;
      }

      return i.evaluate(this.ast);
    }, e.prototype.createGlobalScope = function (e) {
      var t = new p.default("function");
      t.sandbox = new c.default(e);

      for (var i = Object.keys(e), r = 0, n = Object.keys(u.default); r < n.length; r++) {
        var s = n[r];
        i.indexOf(s) < 0 && (t.declaration[s] = l.createSimpleValue(u.default[s]));
      }

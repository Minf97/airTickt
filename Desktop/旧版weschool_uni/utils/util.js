...
const getStorageImage = (web_image) => {
    let webImages = uni.getStorageSync('webImages') || [];
    ...
    if (webImage) {
        try {
            ...
            uni.setStorageSync('webImages', webImages);
        }
    } else {
        uni.downloadFile({
            url: web_image,
            ...
            success(res) {
                if (res.statusCode === 200) {
                    ...
                    let webImageStorage = uni.getStorageSync('webImages') || [];
                    let storage = {
                        web_path: web_image,
                        local_path: filePath,
                    ...
                    uni.setStorageSync('webImages', webImageStorage);
                }
            }
        });
    }
...
const getweekString = () => {
    ...
    var Date2 = new Date(uni.getStorageSync('configData').timeYear);
    var dayOfWeek = Date1.getDay(); //如果把周日算在一周的最后一天，请加上下面这句
    // dayOfWeek = dayOfWeek == 0 ? 7 : dayOfWeek
    //如果把周日算在一周的第一天，请删除上面这句

    var num = (Date1 - Date2) / 1000 / 3600 / 24;
    var whichWeek = Math.ceil((num + dayOfWeek) / 7); // var whichWeek = Math.ceil((num) / 7);

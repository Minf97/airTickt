			...
			<view class="no-search" v-if="!searchStatus">
				<view class="serach-keywords search-history" v-if="!keyword && historyKeyword.length">
					<view class="h">
						...
						<image class="icon" @tap="clearHistory"
							src="http://nos.netease.com/mailpub/hxm/yanxuan-wap/p/20150730/style/img/icon-normal/del1-93f0a4add4.png">
						</image>
					</view>
					<view class="b">
						<view class="item" @tap="onKeywordTap" :data-keyword="item" hover-class="navigator-hover"
				...
				<view class="serach-keywords search-hot" v-if="!keyword">
					...
					<view class="b">
						<view :class="'item ' + (item.is_hot === 1 ? 'active' : '')" hover-class="navigator-hover"
							@tap="onKeywordTap" :data-keyword="item.keyword" v-for="(item, index) in hotKeyword"
							:key="index">
							{{ item.keyword }}
						</view>
					</view>
				...
				<view class="shelper-list" v-if="keyword">
					<view class="item" hover-class="navigator-hover" @tap="onKeywordTap" :data-keyword="item"
						v-for="(item, index) in helpKeyword" :key="index">{{ item }}</view>
				</view>
			</view>
			...
			<view class="search-result" v-if="searchStatus && goodsList.length">
				<view class="sort">
					...
					<view class="sort-box-category" v-if="categoryFilter">
						<view :class="'item ' + (item.checked ? 'active' : '')" :data-category-index="index"
							@tap="selectCategory" v-for="(item, index) in filterCategory" :key="cate - +item.id">
							{{ item.name }}
						</view>
					</view>
				</view>

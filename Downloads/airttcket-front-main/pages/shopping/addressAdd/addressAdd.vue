	...
	<view>
		<view class="add-address">
			<view class="add-form">
				...
				<view class="form-item">
					<input class="input" :value="address.full_region" :disabled="true" @tap="chooseRegion"
						placeholder="省份、城市、区县" />
				</view>
				<view class="form-item">
					<input class="input" @input="bindinputAddress" :value="address.address"
						placeholder="详细地址, 如街道、楼盘号等" />
				</view>
				<view class="form-default">
					<text @tap="bindIsDefault"
						:class="'default-input ' + (address.is_default == 1 ? 'selected' : '')">设为默认地址</text>
				</view>
			</view>
				...
				<view class="hd">
					<view class="region-selected">
						<view
							...
							v-for="(item, index) in selectRegionList" :key="item.id">
							{{ item.name }}
						</view>
					</view>
				...
				<view class="bd">
					<view class="region-list">
						<view :class="'item ' + (item.selected ? 'selected' : '')" @tap="selectRegion"
							:data-region-index="index" v-for="(item, index) in regionList" :key="item.id">
							{{ item.name }}
						</view>
					</view>
				</view>

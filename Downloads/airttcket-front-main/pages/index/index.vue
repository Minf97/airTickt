		...
		<swiper class="banner" :indicator-dots="true" :autoplay="true" interval="3000" duration="1000">
			<swiper-item v-for="(item, index) in banner" :key="item.id">
				<navigator :url="item.link">
					<image :src="item.image_url" background-size="cover" mode="aspectFit"></image>
				</navigator>
			</swiper-item>
		</swiper>
		<view class="m-menu">
		...
		<view class="a-section a-new" v-if="newGoods.length > 0">
			...
			<view class="b">
				<view class="item" v-for="(item, index) in newGoods" :key="item.id">
					...
					<navigator
						:url="computedUrl(item.category_id, item.id)">
						<image class="img" :src="item.list_pic_url" background-size="cover"></image>
						<text class="name">{{ item.name }}, {{item.category_id}}</text>
